...
The default **Trending** tab provides selectable 7-, 30-, 60-, 90-day, and current-year YTD views. It ranks the 8 most-reported species for the selected window and draws light-AM/dark-PM bars using daily bins for 7 and 30 days, weekly bins for 60 and 90 days, and monthly bins for YTD. Each selected window recalculates its chart limit from the visible bin maximum with 20% headroom. A second chart ranks the 10 highest-producing boats with AM and PM shown separately on the same scale. All totals include kept and released encounters; missing reports remain missing rather than confirmed zero-catch days.
