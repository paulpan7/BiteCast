...
def main() -> None:
    ...
    yesterday = dt.datetime.now(PACIFIC).date() - dt.timedelta(days=1)
    through = min(args.through or yesterday, validation_end)
    ...
    if through >= validation_start:
        source_start = max(validation_start, through - dt.timedelta(days=max(1, args.days) - 1))
    ...
    print(f"Actuals written to {args.output}; modelWasRetrained=false")
